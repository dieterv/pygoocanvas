...
def on_item_view_created(view, item_view, item):
    if isinstance(item, goocanvas.Rect):
        item_view.connect("enter_notify_event", on_enter_notify)
        item_view.connect("leave_notify_event", on_leave_notify)
        item_view.connect("button_press_event", on_button_press)
        item_view.connect("button_release_event", on_button_release)
        item_view.connect("motion_notify_event", on_motion)

def create_drag_box(canvas, root, box_name):
    box = goocanvas.Rect(x=0, y=0, width=10, height=10,
                fill_color="black",
                stroke_color="black",
                line_width=1.0)
    root.add_child(box)
    canvas.set_data(box_name, box)

def create_dimension(canvas, root, arrow_name, text_name, anchor):
    p3 = goocanvas.Polyline(fill_color="black", 
                            start_arrow=True, 
                            end_arrow=True)
    root.add_child(p3)
    ...
    text = goocanvas.Text(x=0, y=0, width=-1, anchor=anchor,
                    fill_color="black",
                    font="Sans 12")
    root.add_child(text)
...
def create_info(canvas, root, info_name, x, y):
    t = goocanvas.Text(x=x, y=y, width=-1, anchor=gtk.ANCHOR_NW,
                fill_color="black",
                font="Sans 14")
    root.add_child(t)
...
def on_enter_notify(view, target, event):
    item = target.get_item()
    item.props.fill_color = "red"
    return True

def on_leave_notify(view, target, event):
    item = target.get_item()
...
def on_button_press(view, target, event):
    ...
    canvas_view = view.get_canvas_view()
    canvas_view.pointer_grab(view, gtk.gdk.POINTER_MOTION_MASK | gtk.gdk.BUTTON_RELEASE_MASK,
                    fleur, event.time)
    return True

def on_button_release(view, target, event):
    canvas_view = view.get_canvas_view()
    canvas_view.pointer_ungrab(view, event.time)
    return True

def on_motion(view, target, event):
